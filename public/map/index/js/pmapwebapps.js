...
function initMaps(html) {
    ...
    MAPLAYER = new L.tileLayer(html, { subdomains: [1, 2, 3], minZoom: 8, maxZoom: 19, storagetype: 0 }).addTo(map);
          for(var i=0;i<mapscope.length;i++){
       MAPaircraft= new L.tileLayer(mapscope[i].MapURL, { opacity:1.0,subdomains: [1, 2, 3], minZoom: 11, maxZoom: 19, storagetype: 0,tiletype:"arcgis" }).addTo(MAThangfei)
          }

	MAPLABELLAYER = new L.tileLayer(_TiandituYXLabelPath, { subdomains: [1, 2, 3], minZoom: 8, maxZoom: 19, storagetype: 0 });
    MAPLABELLAYER.addTo(map);
    var url = GeoserverUrl+"?cql_filter=MCID='"+treeTypes+"'"
    console.log(url)
    // 红线
